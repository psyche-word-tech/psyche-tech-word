...
export default function VagueWordsPage() {
	...
	const fetchVagueWords = async () => {
		try {
			 ...
			 * 接口：GET /api/v1/wordbooks/words_y
			 */
			const response = await fetch(`${process.env.EXPO_PUBLIC_BACKEND_BASE_URL}/api/v1/wordbooks/words_y`);
			const data = await response.json();
			setVagueWords(Array.isArray(data) ? data : []);
		} catch (error) {
		...
		<Screen>
			<View style={styles.container}>
				...
				<View style={styles.statsBar}>
					<Text style={styles.statsText}>共 {vagueWords.length} 个单词</Text>
				</View>
