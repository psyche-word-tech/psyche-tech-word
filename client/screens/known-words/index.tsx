...
export default function KnownWordsPage() {
	...
	const fetchKnownWords = async () => {
		try {
			...
			if (wordsResult.data) {
				const known = wordsResult.data.filter((word: Word) => statusMapData[word.id] === 'x');
				setKnownWords(known);
			}
		} catch (error) {
		...
		<Screen>
			<View style={styles.container}>
				...
				<View style={styles.statsContainer}>
					<Text style={styles.statsText}>共 {knownWords.length} 个已会单词</Text>
				</View>
				...
				{loading ? (
					...
					<View style={styles.emptyContainer}>
						...
						<Text style={styles.emptyHint}>将单词拖入&quot;已会&quot;分类即可添加</Text>
					</View>
				)}
			</View>
		</Screen>
